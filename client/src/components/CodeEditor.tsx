...
import "codemirror/addon/fold/brace-fold"; // Если вы используете языки, такие как JavaScript или C++
import "codemirror/addon/fold/comment-fold"; // Опционально, для folding comment блоков
import "codemirror/addon/fold/foldcode";
import "codemirror/addon/fold/foldgutter";
import "codemirror/addon/fold/foldgutter.css"; // стиль fold gutter
...
export default function CodeEditor({
  ...
  // В компоненте CodeEditor
  useEffect(() => {
    if (editorRef.current) {
      editorRef.current.setOption("theme", theme);
    }
    // другое использование useEffect
  }, [theme]);
  ...
  useEffect(() => {
    if (!editorRef.current) {
      ...
      editorRef.current = Codemirror.fromTextArea(editorId, {
        ...
        foldGutter: true, // Включить  fold gutter
