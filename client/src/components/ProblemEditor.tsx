...
export default function ProblemEditor() {
  ...
  const handleSubmit = useCallback(async () => {
    if (!user) {
      toast.error("Пожалуйста зарегистрируйтесь, чтобы отправить код");
      return;
    }
    let problemId = "";
          ...
          <div className="bg-gray-100 flex-grow flex flex-col items-end p-4 pt-2 min-h-[125px]">
            {drawerState.bottomDrawer === "input" ? (
              ...
              >
                ...
                {drawerState.verdict === "wa" && (
                  <span className="text-red-600">НЕВЕРНЫЙ ОТВЕТ!</span>
                )}
                {drawerState.verdict === "tle" && (
                  <span className="text-red-800">ЛИМИТ ВРЕМЕНИ ИСТЁК</span>
                )}
                {drawerState.verdict === "" && (
                  <span className="text-slate-800">
                    СНАЧАЛА ОТПРАВЬТЕ СВОЙ КОД.
                  </span>
                )}
              </div>
            ) : (
