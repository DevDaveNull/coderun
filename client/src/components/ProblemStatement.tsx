        ...
        <>
          ...
          <h1 className="text-3xl grid place-content-center py-3 border-b capitalize mb-8 text-center">
            ...
            <span className="text-sm font-medium text-gray-700 mt-1  tracking-wider">
              Ограничение по времени: {problem?.timelimit}s
            </span>
          </h1>
          <div>
            <h2 className="text-lg text-gray-800 mb-2">Постановка проблемы</h2>
            <MDEditor.Markdown source={problem?.statement} />
          </div>

          {problem?.input && (
            <div className="my-6">
              <h2 className="text-lg text-gray-800 mb-2">Формат ввода</h2>
              <MDEditor.Markdown source={problem?.input} />
            </div>
          )}
          {problem?.output && (
            <div className="my-6">
              <h2 className="text-lg text-gray-800 mb-2">Формат вывода</h2>
              <MDEditor.Markdown source={problem?.output} />
            </div>
          )}
          {problem?.constraints && (
            <div className="my-6">
              <h2 className="text-lg text-gray-800 mb-2">Ограничения</h2>
          ...
          {sampleTestcase &&
            sampleTestcase.map((item, index) => (
              <div key={index}>
                ...
                {item.explanation && (
                  <div className="my-6">
                    <h2 className="text-lg text-gray-800 mb-2">Пояснения</h2>
