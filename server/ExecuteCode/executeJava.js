...
const executeJava = (filepath, userInput) => {
  return new Promise((resolve, reject) => {
    ...
    let compileError = ""; // Переменная для хранения сообщения об ошибке компиляции. Подумать!!!!!
    ...
    compileProcess.on("close", (code) => {
      if (code !== 0) {
        ...
        }); // Отклонить из-за ошибки компиляции
        return;
      }

      const executeDirectory = path.dirname(filepath); // Каталог, в котором находится файл
        ...
        if (code !== 0) {
          reject({
            ...
            message: `Executation Failed:\n${
              runtimeError || "Неизвестная ошибка времени выполнения"
            }`,
          });
          return;
        }
